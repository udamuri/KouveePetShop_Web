    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <form @submit.prevent="updateData()">
          ...
          <button class="btn btn-primary" v-on:click="updateData()" to="/">Submit</button>
        </form>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    loadData() {
      ...
      axios
        .get("http://localhost:8000/api/pegawai/" + this.$route.params.NIP)
        .then(response => {
          // post value yang dari response ke form
          this.form.NIP = response.data.NIP;
          this.form.nama_pegawai = response.data.nama_pegawai;
          this.form.alamat_pegawai = response.data.alamat_pegawai;
          this.form.tglLahir_pegawai = response.data.tglLahir_pegawai;
          this.form.noTelp_pegawai = response.data.noTelp_pegawai;
          this.form.stat = response.data.stat;
          this.form.password = response.data.stat;
        });
    },
    updateData() {
      ...
      axios
        .put("http://localhost:8000/api/pegawai/" + this.$route.params.NIP, {
          ...
          tglLahir_pegawai: this.form.tglLahir_pegawai,
          noTelp_pegawai: this.form.noTelp_pegawai,
          stat: this.form.stat,
